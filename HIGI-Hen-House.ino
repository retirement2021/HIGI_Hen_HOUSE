...
//#define RETRY_DELAY (5 * MINUTE)           // delay auto retries after a fault (not used)
...
#define LIMIT_OPEN_PIN 15   // Door Limit Switch OPEN, Normally open contacts to GND
#define LIMIT_CLOSE_PIN 16  // Door Limit Switch CLOSE, Normally open contacts to GND
...
//unsigned long lastFaultRetry = 0;  //Not Used (pairs with #define RETRY_DELAY (5 * MINUTE) )
...
void drawMainDisplay() {
  ...
  if (wifiLevel >= 2) {
    ...
    u8g2.drawCircle(cx, cy, 5, U8G2_DRAW_UPPER_RIGHT);
  }

  // MIDDLE ARC
  if (wifiLevel >= 3) {

    u8g2.drawCircle(cx, cy, 8, U8G2_DRAW_UPPER_RIGHT);
  }

  // OUTER ARC
  if (wifiLevel >= 4) {

    u8g2.drawCircle(cx, cy, 11, U8G2_DRAW_UPPER_RIGHT);
  }
...
void drawEventsPage() {
  ...
  strncpy(line1, eventHistory[index].msg, 20);
  line1[20] = '\0';

  strncpy(line2, eventHistory[index].msg + 20, 20);
  line2[20] = '\0';
...
void refreshDisplayIfNeeded() {
  ...
  unsigned long refreshRate = 60000UL;
