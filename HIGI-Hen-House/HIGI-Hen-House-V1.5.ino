...
// Door opens AFTER sunrise - BST 
int sunriseOpenOffsetBST = -15; //Negative number = BEFORE sunrise.

// Door closes AFTER sunset - BST
int sunsetCloseOffsetBST = 60; // **

//------------------------------------
//LIGHT BST TIME (Summer)
//-------

// Coop light turns ON before sunset - BST
int lightOnOffsetBST = -15; //Negative number = AFTER sunset.

// Coop light turns OFF after sunset - BST **(Example: Door close offset 60 + 5 minutes = 65)
int lightOffMinutesBST = 65;

//-------------------------------------
//  DOOR GMT TIME. (winter)
//-------

// Door opens AFTER sunrise - GMT 
int sunriseOpenOffsetGMT = 15; //Negative number = BEFORE sunrise.


// Door closes AFTER sunset - GMT
...
// Coop light turns ON before sunset - GMT
int lightOnOffsetGMT = 10;  //Negative number = AFTER sunset.

// Coop light turns OFF after sunset - GMT  **(Example:** Door close offset 30 + 5 minutes = 35)
...
const int ENVIRONMENT_LOG_DAYS = 90;
...
#define MOTOR_TIMEOUT (15 * SECOND)  // adjust for how long the door motor runs between limit switches. \
                                     // check Event page to see how many seconds the door takes, eg 12.4S, Plus 1-2 seconds.
...
#define BUZZER_PIN 3  // Active Buzzer for alert fault and time out only. Connect to GND
...
#define NIGHT_LED1_PIN 11  // RED LED to GND. Installed above the Coop door for visual referance. 100R resistor.
#define NIGHT_LED2_PIN 12  // RED LED to GND. Installed above the Coop door for visual referance.  100R resistor.
...
//#define pin 14  //SPARE, possible frost stat
...
//INFO: Temperature/Humidity. DHT22
// pin 13 = data
// VCC      3.3v
// GND
...
void drawEnvironmentPage() {
  ...
  // DEBUG: Print the environmentLogIndex value

  //Serial.print("environmentLogIndex = ");
  //Serial.println(environmentLogIndex);
  ...
  // =====================================================
    ...
    //u8g2.drawStr((128 - dateWidth) / 2, 28, d.dateString);
...
void handleEventScroll() {
  ...
  if (!buttonDown && buttonWasDown) {
    ...
    if (pressTime < 500) {
      ...
      // NORMAL PAGE CHANGE
      // -------------------------------------------------
      currentPage = (DisplayPage)((currentPage + 1) % 6);  // %_,how many pages in enum block

      viewedEventOffset = 0;
      environmentLogIndex = -2;  //which page to show first,-0=log,-1=summary,-2=current
    ...
    // =====================================================
      ...
      if (manualLightOverride) {
        ...
        lightOn = true;
        digitalWrite(LIGHT_PIN, HIGH);
        manualLightOffTime = millis() + manualLightTimeout;
...
void updateNightLeds() {
  ...
  unsigned long flashInterval = BEDTIME_FLASH_MS;

  if (nightMode) {
    flashInterval = NIGHT_FLASH_MS;
  } else if (bedtimeWarning) {
    flashInterval = BEDTIME_FLASH_MS;
  }
