...
// Seven display pages. Main, Timings, Environment, Events, wifi, today's scedule and a interactive manual page for opening/closing the door and light on/off
// Temperature and humidity in the coop recorded for 30 days in non volitile memory
// Events recorded in volitile memory.
...
const char systemVersion[] = "V1.22";
...
int lightOnOffsetGMT = -20; // __ minutes before sunset
...
const unsigned long STAGE0_FLASH_MS = 800;   // Stage 0: Slower flash. defaults 500 | 800. Bug Note: Applying *50 triggers a known issue across all stages.
...
// DHT22 Sensor
#define MAX_CONSECUTIVE_DHT_FAILURES 10        // Log fault after 10 consecuitive failures
...
enum DisplayPage {
  ...
  PAGE_SCHEDULE,     // Today's door & light time info
  PAGE_ENVIRONMENT,  // Temp & humidity info, scroll for more pages
  PAGE_MANUAL,       // Manual door open/close, light on/off
...
void drawSchedulePage() {
  ...
  // Calculate times (minutes since midnight). openTime & closeTime are maintained by updateSunTimes()
  int doorOpenMinutes = openTime;    // may be fixed or sunrise+offset depending on config
  int doorCloseMinutes = closeTime;  // scheduled close (start of closing)
  ...
  // Draw four equally spaced rows using the same font
  u8g2.setFont(u8g2_font_6x12_tf);

  const int xLabel = 6;
  const int rightPad = 6;
  const int yStart = 28; // first baseline for line 1
  const int yStep = 12;  // equal spacing matching font height
