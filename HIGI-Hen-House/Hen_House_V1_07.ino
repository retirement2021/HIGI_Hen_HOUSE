...
// All customisation is placed together in one block split into two parts ***USER ADJUSTABLE SETTING*** & **USER ADJUSTABLE MAGIC NUMBERS*** 
...
// Soltice twilight end +45 minutes. Default -10
int lightOnOffsetBST = -15;

//LIGHT OFF
// Coop light OFF offset from sunset - BST
// Soltice twilight end +45 minutes. Default +35
// **** Example: Door closing at +6, plus 2 minutes +8.(light turns off 2 minutes after door closes)
int lightOffMinutesBST = +8;
...
// Soltice twilight end +37 minutes. Default +10
// **** See light OFF
int sunsetCloseOffsetGMT = +10;
...
// **** Example: Door closing at +10, plus 2 minutes = 12 minutes.(light turns off 2 minutes after door closes)
int lightOffMinutesGMT = +12;
...
//--------------------------------------
// Progressive bedtime flash stages (visual chicken homing). keep stage 1 time smaller than Door closing offset.
...
const unsigned long NIGHT_FLASH_MS = 1200;   // Alternating flash. When door is fully closed.
...
void updateNightLeds() {
  ...
  // =====================================================
    ...
    if (getTime(timeinfo)) {
      ...
      // Stage 3:  fastest flash (continues until door closes) 
      if (minutesUntilClose <= BEDTIME_STAGE3_MINS) {
         flashInterval = STAGE3_FLASH_MS;
      }

      // stage 2: fast flash. 
      else if (minutesUntilClose <= BEDTIME_STAGE2_MINS && minutesUntilClose > BEDTIME_STAGE2_MINS) {
        flashInterval = STAGE2_FLASH_MS;
      }

      // Stage 1: medium flash. 
      else if (minutesUntilClose <= BEDTIME_STAGE1_MINS && minutesUntilClose > BEDTIME_STAGE1_MINS) {
        flashInterval = STAGE1_FLASH_MS;
      }

      // Stage 0: After delay - slow flash
