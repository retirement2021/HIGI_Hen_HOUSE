...
const char systemVersion[] = "V1.07";
...
//  Soltice twilight start -45 minutes. Default +15
int sunriseOpenOffsetBST = +15;

// Door closing offset from sunset - BST
// Soltice twilight end +45 minutes. Default +30
// ****
...
// Soltice twilight end +45 minutes. Default -15
int lightOnOffsetBST = -15;

// Coop light OFF offset from sunset - BST
// Soltice twilight end +45 minutes. Default +35
// **** Example: Door closing +30 plus 5 minutes =35
...
// **** Example: Door closing +20 plus 5 minutes = 25 minutes
int lightOffMinutesGMT = +25;
...
// Progressive bedtime chicken homing stages
const unsigned long BEDTIME_STAGE0_DELAY_MINS = 1;  // Stage 0: Minutes after coop light comes ON
const unsigned long BEDTIME_STAGE1_MINS = 15;       // Stage 1: Minutes before door close
const unsigned long BEDTIME_STAGE2_MINS = 5;        // Stage 2: Minutes before door close

const unsigned long STAGE0_FLASH_MS = 550;  // Stage 0: Slower flash (1000ms = 1 second)
const unsigned long STAGE1_FLASH_MS = 400;  // Stage 1: Medium flash
const unsigned long STAGE2_FLASH_MS = 250;  // Stage 2: Faster flash

const unsigned long NIGHT_FLASH_MS = 1000;  // Door secured, sequence flash. Starts when the lower limit switch is activated (fully closed)
...
unsigned long oledTimeout = (2 * MINUTE);
...
enum DisplayPage {
  ...
  PAGE_MAIN,         // Info Only: Short push changes page
  PAGE_MANUAL,       // Interactive: CW 3 detents close door, CCW 3 detents open door & push for 2 seconds toggles coop light ON/OFF
...
DisplayPage currentPage = PAGE_MAIN;  // Always 1st page

//======================================================
//*******USER ADJUSTABLE MAGIC NUMBERS*******
//======================================================
...
#define MOTOR_TIMEOUT (15 * SECOND)  // Adjust for how long the door motor runs between the door limit switches (from fully closed to fully open position) \
                                     // Check Event page, eg "Door Open 13.9s", "Door Close 13.2s", plus 1-2 seconds. Keep this tight.

#define MOTOR_TIMEOUT_RECOVERY (18 * SECOND)  // Only triggered when door obstruction is detected. Difference between MOTOR_TIMEOUT and MOTOR_TIMEOUT_RECOVERY. eg additional 3 seconds. \
                                              // Allows for more time for cord unwinding and rewinding, eliminating MOTOR_TIMEOUT fault. Add a >=150g weight to the cord to maintain cord tension.

//-------------------------------------

// Door Obstruction Timing
#define SAFETY_WAIT_AFTER_REOPEN_MS (5 * SECOND)  // Hold door open for 5 seconds before retry of door obstruction close. Allows chicken to move out of the way.

// Display Pages
#define NUM_DISPLAY_PAGES 6  // number of display pages

// Rotary Encoder
#define ENCODER_DETENTS 3  // Encoder detents for door open/close on manual page

// DHT22 Sensor
#define MAX_CONSECUTIVE_DHT_FAILURES 10        // Log fault after 10 failures (~50 minutes)
#define DHT_FIRST_READ_DELAY_MS (30 * SECOND)  // Delay before first reading (30 seconds for settling)
#define DHT_READ_INTERVAL_MS (5 * MINUTE)      // Reading interval (5 minutes)

// WiFi
#define WIFI_RETRY_INTERVAL (90 * SECOND)  // if WiFi fails, retry every __ until connected
#define MAX_WIFI_RETRIES 20                // Give up WiFi after this many attempts. (30 minutes)
...
#define LIGHT_PIN 17  // Auto Coop Light. To control a logic level relay board module
...
void updateSunTimes(struct tm timeinfo) {
  ...
  int seasonalSunriseOffset = getSeasonalOffset(sunriseOpenOffsetBST, sunriseOpenOffsetGMT, timeinfo);
  int seasonalSunsetOffset = getSeasonalOffset(sunsetCloseOffsetBST, sunsetCloseOffsetGMT, timeinfo);

  openTime = actualSunrise + seasonalSunriseOffset;
...
void updateNightLeds() {
  ...
  // =====================================================
  // BEDTIME WARNING - Progressive 3 stages
  // =====================================================
    ...
    if (getTime(timeinfo)) {
      ...
      // Stage 2: Last 5 minutes before close - fastest flash
      if (minutesUntilClose <= BEDTIME_STAGE2_MINS && minutesUntilClose > 0) {
        flashInterval = STAGE2_FLASH_MS;
      }
      // Stage 1: 15 to 5 minutes before close - medium flash
      else if (minutesUntilClose <= BEDTIME_STAGE1_MINS && minutesUntilClose > BEDTIME_STAGE2_MINS) {
        flashInterval = STAGE1_FLASH_MS;
      }
      // Stage 0: After delay to 15 minutes before close - slow flash
