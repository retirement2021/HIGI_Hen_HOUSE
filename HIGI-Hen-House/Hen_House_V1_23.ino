...
// V1.23 re-wrote update-Environment to reduce spurious temperatures being logged.
...
const char systemVersion[] = "V1.23";
...
void updateEnvironment() {
  ...
  const int DHT_READ_RETRIES = 3;
  const unsigned long DHT_RETRY_DELAY_MS = 250;
  ...
  // Validate ranges: keep within plausible DHT22 bounds
  bool valid = true;
  ...
  if (!valid) {
    ...
    Serial.print("DHT22 Read Invalid (attempts=");
    Serial.print(DHT_READ_RETRIES);
    Serial.print(") temp=");
    Serial.print(t);
    Serial.print(" hum=");
    Serial.println(h);
