...
// V2.0 Replace DHT22 with SHT31 temperature humidity sensor

// Requires a minimal wifi signal to set NTP time.
// Requires a permanent 12v power source.
// The system uses a geared motor and cord guillotine style door, cheap and reliable.
// All customisation is placed together in one block split into two parts ***USER SETTINGS*** & ***USER MAGIC NUMBERS*** close to the top of the sketch
...
// System controlled via a rotary encoder. Short press <500ms,wake and page change, long press >=5 seconds fault reset.
...
//MORNING.
// Only used when "DOOR OPENING OPTION" is set to "false". (GMT Winter only)
...
int lightOnOffsetGMT = -15;  // __ minutes before sunset

//LIGHT OFF
// Coop light OFF offset from sunset - GMT
int lightOffMinutesGMT = +10;
...
//  MORNING -
//  Only used when "DOOR OPENING OPTION" is set to "false". (BST summer only)
...
const unsigned long BEDTIME_STAGE1_MINS = 9;        // Stage 1: Minutes before door close. defaults 9 | 7
const unsigned long BEDTIME_STAGE2_MINS = 5;        // Stage 2: Minutes before door close. defaults 5 | 4
const unsigned long BEDTIME_STAGE3_MINS = 1;        // stage 3: Minutes before door close. defaults 1 | 1

// Progressive bedtime flash speed. (1000  milli-seconds = 1 second)

const unsigned long STAGE0_FLASH_MS = 800;  // Stage 0: Slower flash. defaults 500 | 800. Bug Note: Keep values ending in *00 to avoid the known issue across all stages & Night_flash.
const unsigned long STAGE1_FLASH_MS = 600;  // Stage 1: Medium flash. defaults 400 | 600.
const unsigned long STAGE2_FLASH_MS = 400;  // Stage 2: Faster flash. defaults 300 | 400.
const unsigned long STAGE3_FLASH_MS = 200;  // Stage 3: Quick  flash. defaults 200 | 200.


// Visual Door Closed.
// The door must trigger the lower limit switch to initiate a fully closed state before the Night Alternating Flash starts.

const unsigned long NIGHT_FLASH_MS = 1200;  //Continues to flash all night until the door opens in the morning. Default 1200
...
#define MAX_EVENTS 100  // default 100

//--------------------------------------
// ENVIRONMENT LOG ENTRIES
//--------------------------------------
// Number of persistent days stored.
const int ENVIRONMENT_LOG_DAYS = 30;  // Default 30
...
#define SHT_FIRST_READ_DELAY_MS (30 * SECOND)   // On power up delay before first reading (~30 seconds for sensor settling)
#define SHT_READ_INTERVAL_MS (13 * MINUTE)      // Subsequent reading interval every __ minutes (9 minutes=160, 13 minutes=110 reads in 24hrs)
...
// Rotary Encoder
#define ENCODER_DETENTS 3  // Encoder detents for door open/close on the manual page. To prevent accidental initializing
...
enum DisplayPage {
  ...
  PAGE_ENVIRONMENT,  // Temp & humidity info. Scroll info
...
// Display Pages
#define NUM_DISPLAY_PAGES 6  // number of display pages shown. eg First __ pages shown. Default 7
...
//#define pin 14  //SPARE, possible DHT22 external frost stat with door opening delay.

// =======================================================
// ADDITIONAL GPI0 PIN INFO
// =======================================================

// INFO: 1.3" OLED display, SHT31 temperature sensor and RTC DS3231 are all I2C protocol, Use the same connection pins for all three devices.
// Pin 8 = SDA
// Pin 9 = SCK/SCL
// VCC      3.3v or 5V
// GND
...
// pin 17 = data
// VCC      3.3v or 5v depending relay coil voltage
// GND
...
void drawSystemTimePage() {
  ...
  if (timeValid && isInBST(timeinfo) && useFixedOpenTime) {
    ...
    // GMT (not BST) and GMT fixed open enabled
    snprintf(
      sunBuf,
      sizeof(sunBuf),
      "Open %d:%02d SS %02d:%02d",
...
void updateEnvironment() {
  ...
  for (int attempt = 0; attempt < SHT_READ_RETRIES; attempt++) {
    ...
    if (!isnan(t) && !isnan(h) && isfinite(t) && isfinite(h)) {
      break; // valid
    }
  ...
  // Validate ranges: keep within plausible SHT31 bounds
  bool valid = true;
  ...
  if (!valid) {
    ...
    Serial.print("SHT31 Read Invalid (attempts=");
...
void setup() {
  ...
  if (!sht31.begin(0x44)) {  // try 0x44 (or 0x45 if your module uses that)
    setEvent("SHT31 NOT FOUND");
    Serial.println("SHT31 NOT FOUND");
    // sensor not found — code continues but reads will fail and be counted as invalid
  } else {
    setEvent("SHT31 OK");
