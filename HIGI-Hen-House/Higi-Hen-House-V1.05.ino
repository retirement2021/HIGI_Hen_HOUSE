...
const char systemVersion[] = "V1.05";
...
//--------------------------------------
const unsigned long BEDTIME_FLASH_MS = 400;  // Sunset warning pulse in milliseconds

const unsigned long NIGHT_FLASH_MS = 1000;  // Door secured sequence flash in milliseconds
...
//#define pin 14  //SPARE, possible DHT22 frost stat.
...
void updateEnvironment() {
  ...
  if (millis() - lastRead < 300000) { // DHT22 reads every 5 minutes, 288 checks in 24hrs
...
void updateDoor() {
  ...
  if (obstructionRetries >= MAX_OBSTRUCTION_RETRIES) {
    ...
    // RESET and start new safety cycle
    safetyStartTime = millis();  // ← Reset timer for next retry
  ...
  switch (safetyState) {
    ...
    case SAFETY_WAITING:
      ...
      if (millis() - safetyOpenReachedTime > 5000UL) {
        startClosing();
        safetyState = SAFETY_IDLE;  // ← Only reset here after starting close
      }
      break;
  }
