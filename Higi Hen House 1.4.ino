...
void drawEnvironmentPage() {
  ...
  if (environmentLogIndex < 0) {
    ...
    u8g2.setFont(u8g2_font_7x14_tf);  // LARGER FONT
...
void handleEventScroll() {
  ...
  if (currentPage == PAGE_ENVIRONMENT) {
    ...
    if (encoderValue != lastEncoderValue) {
      ...
      if (delta < 0) {
        ...
        if (environmentLogIndex < -1) {
          environmentLogIndex = -29;
        }
      }
  ...
  if (!buttonDown && buttonWasDown) {
    ...
    if (pressTime < 500) {
      ...
      // NORMAL PAGE CHANGE
      // -------------------------------------------------
      currentPage = (DisplayPage)((currentPage + 1) % 6);
